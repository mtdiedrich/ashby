...
import { resumeFile, resumeBlock } from './resume.js';

const CACHE = () => P.resumeText;
const MODEL = 'claude-haiku-4-5';

export async function resumeText() {
  if (!fs.existsSync(resumeFile())) throw new Error(`${resumeFile()} not found`);
  const fileHash = crypto.createHash('sha256').update(fs.readFileSync(resumeFile())).digest('hex').slice(0, 16);
  ...
  if (!doc) throw new Error(`cannot read ${resumeFile()} as a document`);

  console.log(`transcribing ${resumeFile()} with ${MODEL}...`);
