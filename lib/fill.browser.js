...
(function () {
  ...
  // ---- postal address ----------------------------------------------------
  //
  // Scanned 178 live application forms: an address field appears on about 1% of them,
  // there was no separate zip field on any of them, and both real ones were TEXTAREAS
  // wanting a whole mailing address rather than a street line. So me.json stores the
  // street and postcode separately and they are composed here.
  //
  // The city and state come from ME.location, which already exists for the location
  // combobox, so an address is not a second place to keep the same thing.

  // "Email Address" is the trap. It appeared MORE often in the scan than real address
  // fields did. The email rule normally claims it first, but only when an email is
  // configured — an unset one leaves the field untouched and free for this rule, which
  // would then type a street address into it. So it is excluded here too.
  const NOT_AN_ADDRESS = /e-?mail|website|\burl\b|\bip\b|linked ?in|github|twitter|web ?site|line ?2|address ?2/i;

  function isAddressField(label) {
    const t = String(label ?? '');
    if (NOT_AN_ADDRESS.test(t)) return false;
    return /\baddress\b|\bstreet\b/i.test(t);
  }

  function isPostalField(label) {
    return /\bzip\b|\bzip\/|postal code|post ?code\b/i.test(String(label ?? ''));
  }

  /**
   * The mailing address as one string, or '' when nothing is configured.
   * multiline: true for a textarea, false for a single-line input.
   */
  function mailingAddress(me, { multiline = false } = {}) {
    me = me || {};
    const street = String(me.street ?? '').trim();
    const city = String(me.location ?? '').trim();
    const zip = String(me.postalCode ?? '').trim();

    // The postcode belongs with the city, not on a line of its own.
    const cityLine = [city, zip].filter(Boolean).join(' ');
    return [street, cityLine].filter(Boolean).join(multiline ? '\n' : ', ');
  }
  ...
  // RULES match with .test(); these two need a predicate rather than one pattern,
  // so they are wrapped to keep every other rule's shape unchanged.
  const isAddressField_re = { test: l => isAddressField(l) };
  const isPostalField_re  = { test: l => isPostalField(l) };
  ...
  const RULES = [
    ...
    { name: 'postcode',    match: isPostalField_re,                                     kind: 'text',   value: () => ME.postalCode },
    { name: 'address',     match: isAddressField_re,                                    kind: 'address', value: () => mailingAddress(ME) },
  ...
  // A textarea gets the address over lines; a single-line input gets it comma-joined.
  async function fillAddress(en) {
    const el = textInput(en);
    const multiline = !!el && el.tagName === 'TEXTAREA';
    const text = mailingAddress(ME, { multiline });
    if (!text) throw new Error('no street/location configured in me.json');
    return fillText(en, text);
  }
  ...
  async function applyValue(en, kind, value) {
    if (kind === 'address')  return fillAddress(en);
  ...
  window.__ashby = {
    ...
    _: { allEntries, byPath, labelOf, kindOf, isEmpty, isRequired,
         ...
         fillPay, salaryAsk, declineIndex, isDemographic,
         fillAddress, mailingAddress, isAddressField, isPostalField, RULES },
  };
})();
