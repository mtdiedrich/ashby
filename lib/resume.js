...
/** Resolved per call so ASHBY_RESUME and ASHBY_HOME are honoured at run time. */
export const resumeFile = () => resumePath();
...
export function resumeBlock() {
  ...
  if (!fs.existsSync(resumeFile())) {
    console.warn(`⚠  ${resumeFile()} not found — the model will score and write without your resume.`);
    _block = false;
    return null;
  }

  const ext = path.extname(resumeFile()).toLowerCase();
  const media = MEDIA[ext];
  if (!media) {
    console.warn(`⚠  ${resumeFile()} is ${ext}, which the API does not accept as a document. ` +
                 `Export a PDF (or set ASHBY_RESUME to one) — continuing without it.`);
    _block = false;
    return null;
  }

  const data = fs.readFileSync(resumeFile());
  _block = {
    type: 'document',
    title: 'Resume',
